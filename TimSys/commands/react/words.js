const ElizaHelper = require('./eliza.js');
...
module.exports = (client, args, Discord) => {
    ...
    .then((res) => {
        ...
        console.log(chalk.cyan('[' + DateTime.utc().toFormat(timeFormat) + '][Sellout]', chalk.white(`Sellout list initialized from remote, ${selloutList.length} items`)));
    });
    ...
    let doSelloutMessage = (channel) => {
        ...
        try {
            ...
            console.error('[' + DateTime.utc().toFormat('MM/DD/YYYY-h:mm:ss-A') + '][Sellout] ERR:', e.toString());
        }
    };
    //Commands
    ...
    client.on('messageCreate', message => {
        ...
        try {
            ...
            let fnTextReply = function (txt, force, asNormal) {
                if (okayToTextReply || force) {
                    try {
                        ...
                        console.error('[' + DateTime.utc().toFormat('MM/DD/YYYY-h:mm:ss-A') + '][Chat]', 'Reply error:', e)
                    }
                }
            ...
            console.error('[' + DateTime.utc().toFormat('MM/DD/YYYY-h:mm:ss-A') + '][Chat]', 'Message processing / dumb joke error:', e, `<<< ${e.toString()} >>>`);
        }
    });
}
